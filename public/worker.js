...
var goFunc;
self.setGoFunc = callback => {
  goFunc = callback;
  console.log("W: Ready to roll");
...
async function train(input) {
  console.log("W: Training... ");
  await goFunc("train", input);
  self.postMessage({ status: "done" });
}

async function generate(length) {
  console.log("W: Generating...");
  await goFunc("generate", length);
  self.postMessage({ status: "doneGen", output: self.output });
}

self.addEventListener("message", function(e) {
  if (e.data.func == "start") {
    start();
  }
  if (e.data.func == "train") {
    train(e.data.input);
  }

  if (e.data.func == "generate") {
    generate(e.data.length);
  }
});
